...
exports.default = WebpackIsomorphicToolsPlugin;
...
// a Webpack plugin
function WebpackIsomorphicToolsPlugin(options) {
...
WebpackIsomorphicToolsPlugin.prototype.apply = function (compiler) {
	...
	var onDone = function onDone(stats) {
		return writeAssets(stats, _this2, webpack_configuration);
	};
...
function writeAssets(stats, plugin, webpack_configuration) {
	...
	var webpack_assets_path = _path2.default.resolve(options.project_path, options.webpack_assets_file_path);

	// resolve webpack-stats.json file path
	var webpack_stats_path = _path2.default.resolve(options.project_path, options.webpack_stats_file_path);
	...
	if (options.development && options.verbosity !== _common.verbosity_levels.no_webpack_stats) {
		// outputs stats info to the console
		// (only needed in development mode)
		(0, _notifyStats2.default)(stats, json, options.verbosity === _common.verbosity_levels.webpack_stats_for_each_build);
	}
	...
	var serve_assets_from_memory = options.development && options.port;
